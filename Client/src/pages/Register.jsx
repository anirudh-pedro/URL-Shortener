...
import { FiUser, FiMail, FiLock, FiLink, FiArrowRight, FiCheck } from 'react-icons/fi';
...
const Register = () => {
  ...
  const getPasswordStrength = (pwd) => {
    if (!pwd) return { score: 0, text: 'Empty', color: 'bg-slate-800' };
    if (pwd.length < 6) return { score: 1, text: 'Weak (min 6 chars)', color: 'bg-rose-500' };
    ...
    if (hasUpperCase && hasNumbers) strength = 3;
    if (hasUpperCase && hasNumbers && hasSpecial && pwd.length >= 8) strength = 4;

    if (strength === 2) return { score: 2, text: 'Fair', color: 'bg-amber-500' };
    if (strength === 3) return { score: 3, text: 'Good', color: 'bg-violet-500' };
      ...
      <div className="w-full md:w-[45%] lg:w-[40%] flex items-center justify-center p-6 relative">
        ...
        >
          ...
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center p-3 bg-slate-50 border border-slate-200 rounded-2xl mb-4 text-slate-805 shadow-inner md:hidden">
              <FiLink className="w-6 h-6" />
            </div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Create Account</h2>
            <p className="text-xs text-slate-500 mt-1 font-light">Shorten links in seconds</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Full Name */}
            <div>
              <label className="block text-[10px] font-semibold text-slate-550 uppercase tracking-widest mb-2">
                Full Name
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:border-slate-400 focus:ring-1 focus:ring-slate-400 text-sm text-slate-800 placeholder-slate-400 focus:outline-none transition-all duration-300"
                  disabled={submitting}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-550/10 border border-slate-200 focus:border-slate-400 focus:ring-1 focus:ring-slate-400 text-sm text-slate-800 placeholder-slate-400 focus:outline-none transition-all duration-300"
                  disabled={submitting}
                />
              </div>
            </div>
            ...
            <div>
              ...
              {password && (
                <div className="mt-2.5">
                  <div className="flex items-center justify-between text-[9px] text-slate-500 font-semibold mb-1 uppercase tracking-wider">
          ...
          <p className="text-center text-xs text-slate-550 mt-6 font-light">
            Already registered?{' '}
            <Link to="/login" className="text-violet-650 hover:underline font-semibold ml-1">
              Sign in
            </Link>
          </p>
        </motion.div>
